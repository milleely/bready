    ...
    <Card className="border border-stone-200 bg-card shadow-sm dark:border-stone-800">
      <CardContent className="pt-6 flex flex-col h-full">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <div>
            <h2 className="text-lg font-semibold text-stone-900 dark:text-stone-100">Your Progress</h2>
            <p className="text-sm text-stone-600 dark:text-stone-400">{displayMonth} - Actual spending vs budget allocation</p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {allocations.map((allocation) => {
            ...
            return (
              <div
                key={allocation.key}
                className="p-4 rounded-lg bg-stone-50 dark:bg-stone-900/50 border border-stone-200 dark:border-stone-800"
              >
                {/* Header Row */}
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-stone-900 dark:text-stone-100">{allocation.label}</p>
                      <p className="text-xs text-stone-500 dark:text-stone-400">{allocation.percentage}%</p>
                    </div>
                  </div>
        ...
        {/* Category Breakdown */}
        <div className="mt-4 p-4 rounded-lg bg-stone-50 dark:bg-stone-900/50 border border-stone-200 dark:border-stone-800">
          <h3 className="text-sm font-semibold text-stone-900 dark:text-stone-100 mb-3">📊 Category Breakdown</h3>
          ...
          <div className="mb-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-semibold text-blue-600 dark:text-blue-400">Needs (Essential)</span>
            </div>
            <div className="pl-5 flex flex-wrap gap-2">
              {EXPENSE_TO_BUDGET_MAPPING.needs.map((category) => (
                <span
                  key={category}
                  className="text-xs px-2 py-1 rounded-md bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800"
                >
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-semibold text-amber-600 dark:text-amber-400">Wants (Discretionary)</span>
            </div>
            <div className="pl-5 flex flex-wrap gap-2">
              {EXPENSE_TO_BUDGET_MAPPING.wants.map((category) => (
                <span
                  key={category}
                  className="text-xs px-2 py-1 rounded-md bg-amber-50 dark:bg-amber-950/30 text-amber-700 border border-amber-200 dark:border-amber-800"
                >
                  {category.charAt(0).toUpperCase() + category.slice(1).replace('-', ' ')}
                </span>
              ))}
